...
def _meta2autofield_dict(meta, val2str=True, schema=None, consider_ucn=True):
    ...
    def _deep_kv(basekey, dict_or_list_or_value):
        ...
        if isinstance(dict_or_list_or_value, list):
            for list_element in dict_or_list_or_value:
                for i in _deep_kv(basekey, list_element):
                    yield i
            return

        # We know that dict_or_list_or_value is a dict now.
        if '@list' in dict_or_list_or_value:
            # Handle the dictionary-based list representation of JSON-LD here
            for list_element in dict_or_list_or_value['@list']:
                for i in _deep_kv(basekey, list_element):
                    yield i
            return
        ...
        if '@graph' in dict_or_list_or_value:
            ...
            for list_element in dict_or_list_or_value['@graph']:
                for i in _deep_kv(basekey, list_element):
                    yield i
            return
        ...
        for k, v in dict_or_list_or_value.items():
            if k in ('@type', 'datalad_unique_content_properties') or (basekey != '' and k == '@id'):
                continue
            key = _encode_key(k)
            new_basekey = u'{}.{}'.format(basekey, key) if basekey else u'{}'.format(key)
            for i in _deep_kv(new_basekey, v):
                yield i
        return
